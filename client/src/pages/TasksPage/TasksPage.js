...
const TasksPage = () => {
  ...
  const onDeleteHandler = async tid => {
    try {
      const response = await sendRequest(`${API_URL}/tasks/${tid}`, 'DELETE', {
        userId: userId,
      });
      setLoadedTasks(response.tasks);
    } catch (err) {
  ...
  const updateStatusTask = async (tid, status) => {
    try {
      const response = await sendRequest(`${API_URL}/tasks/${tid}`, 'PUT', {
        userId: auth.userId,
        completed: !status,
      });
  ...
  useEffect(() => {
    const filterHandlerForUseEffect = () => {
      switch (isFiltered) {
        ...
        case 'Active':
          setFilteredTasks(
            loadedTasks.filter(task => task.completed === false)
          );
          break;
        default:
        ...
        <Card>
          {!isLoading && loadedTasks.length === 0 && (
            <div className="tasks__message">
              <p
                className={`tasks__content-message tasks__content-message--${theme.theme}`}
              >
          ...
          <Card>
            <div className="tasks__wrapper-container">
              ...
              <Button
                ...
                onClick={onfilterHandler}
                value="Active"
                active={isFiltered === 'Active' && 'active'}
              />
              <Button
                type="button"
                title="Completed"
                ...
                onClick={onfilterHandler}
                value="Completed"
                active={isFiltered === 'Completed' && 'active'}
              />
            </div>
          </Card>
        </div>
